...
|-------|---------|---------------|
| BotAwake sleep lock | `pmset -g \| rg SleepDisabled` | Should be **`0`** in Normal. If **`1`**, run `sudo pmset -a disablesleep 0` or quit and reopen BotAwake (v1.0.1+). |
...
| 检查项 | 命令 | 含义 |
|--------|------|------|
| BotAwake 睡眠锁 | `pmset -g \| rg SleepDisabled` | Normal 下应为 **`0`**。若为 **`1`**，运行 `sudo pmset -a disablesleep 0` 或退出并重新打开 BotAwake（v1.0.1+）。 |
